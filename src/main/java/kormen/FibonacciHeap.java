...
public class FibonacciHeap<T> {
    ...
    public Entry<T> enqueue(T value, double priority) {
        ...
        /* Increase the size o the heap; we just added something */
        ++mSize;
    ...
    /**
     * Returns an Entry object corresponding to the minimum elemtn of the
     * Fibonacci heap, throwing a NoSuchElementException if the heap is empty.
     *
     * @return The smalles element of the heap
    ...
    public Entry<T> dequeueMin() {
         ...
         * are two cases to consider. First, if this is the only element in the list of roots,
         * we set the list of roots to be null by clearing mMin.
         * Otherwise, if it's not null, then we write the elements next to the min element
         * around the min element to remove it, then arbitrarily
         * reassign the min
         */
        ...
        /* Next, clear the parent fields of all of the min element's
        children, since they're about to become roots. Because the elements
        are stored in a circular list, the traversal is a bit complex.
         */
        ...
        traversal order mid-stream. One makor challenge is how to detect
        ...
        for (Entry<T> curr : toVisit) {
            while (true) {
                /*
                Ensure that the list is long enough to hold an elemnt of this
                degree
                 */
                ...
                treeTable.set(curr.mDegree, curr); //Clear the slot
                ...
                /* Clear max's mark, since it can nw lose another child */
                max.mIsMarked = false;
     ...
     * @throws java.lang.IllegalArgumentException if the new priority exceeds the old
    ...
    private void decreaseKeyUnchecked(Entry<T> entry, double priority) {
        ...
        /*
        If the node no longer has a higher priority that its parent, cut it.
        ...
        decreasing the key in a way that ties the current minimum priority,
        this will change the min accordingly
         */
    ...
    /**
     * Cut a node from its parent. If the parent was already marked, recursively
    ...
    private void cutNode(Entry<T> entry) {
        ...
        /* Rewire the node's siblings aroung it, if it has any siblings */
